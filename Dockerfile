# Builds a Docker image to run decidr with the uWSGI application server
...
RUN ln -s /var/www/app/static /var/www/decidr-static
VOLUME /var/www/decidr-static
...
#CMD ["uwsgi", "--http", ":5000",         "--wsgi-file", "decidr.py", "--callable", "app", "--processes",  "2", "--threads", "4"]
CMD ["uwsgi", "--socket", "0.0.0.0:5000", "--wsgi-file", "decidr.py", "--callable", "app", "--processes",  "2", "--threads", "4"]